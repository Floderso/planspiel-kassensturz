...
test('HTML-Struktur: Alle Widget-IDs aus dem Hamburger-Menü existieren als Panel in index.html', () => {
  for (const wid of WIDGET_IDS) {
    ...
    assert.ok(indexHtml.includes(menuMatcher), `Menü-Eintrag für ${wid} fehlt in index.html`);
    assert.ok(indexHtml.includes(panelMatcher), `Widget-Panel für ${wid} fehlt in index.html`);
    assert.ok(indexHtml.includes(removeMatcher), `Entfernen-Button für ${wid} fehlt in index.html`);
  }
});

test('HTML-Struktur: Kabinetts-Leiste, Ressort-Dropdown und Grid-Toggle existieren', () => {
  assert.ok(indexHtml.includes('id="btn-ressort-menu"'), 'btn-ressort-menu fehlt in index.html');
  assert.ok(indexHtml.includes('id="ressort-menu-dropdown"'), 'ressort-menu-dropdown fehlt in index.html');
  assert.ok(indexHtml.includes('id="btn-toggle-ressorts-grid"'), 'btn-toggle-ressorts-grid fehlt in index.html');
  assert.ok(indexHtml.includes('id="btn-toggle-watchlist"'), 'btn-toggle-watchlist fehlt in index.html');
  assert.ok(indexHtml.includes('id="empty-widgets-state"'), 'empty-widgets-state fehlt in index.html');
});

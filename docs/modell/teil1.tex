...
\begin{enumerate}
\item \textbf{Alles ist ein Vergleich.} Die Absolutwerte sind kalibriert, aber die Aussagen
  des Planspiels liegen in den Differenzen. Die Wirkungen in diesem Dokument sind gegen den
  Status-quo-Pfad \emph{derselben} Periode gemessen. Die Engine selbst weist die Änderung des
  Nettoeinkommens je Haushalt gegen den Status quo von 2025 aus; darin stecken auch die
  Änderungen, die ohne jeden Beschluss kommen (steigender Rentenbeitrag nach geltendem Recht,
  fallende \COz-Last auf dem Emissionspfad).
\item \textbf{Verhalten ist berücksichtigt, aber einfach.} Haushalte arbeiten etwas weniger,
  wenn ihnen von einem zusätzlichen Euro weniger bleibt; der besteuerte Konsum sinkt, wenn die
...
(regressiv: bei 55\,\euro/t rund 1,2\,\% des Einkommens in D1, 0,3\,\% in D10c) und die Grenzkonsumneigung. Dass
...
0,35\,\%, getrennt für den Regel- und den ermäßigten Satz. Diese Reaktion wirkt nur auf das
Aufkommen des Staates; die Umsatzsteuerlast der Haushalte rechnet ohne sie
(\datei{tests/buchung.test.js}).
...
Aus den zwölf Nettoeinkommen entstehen die Verteilungsmaße. Gini und Palma werden wie in der
amtlichen Statistik (EU-SILC) auf das \emph{Äquivalenzeinkommen} bezogen: Ein Haushalt mit
vier Personen braucht mehr als einer mit einer, also wird durch das Bedarfsgewicht geteilt.
Anders als EU-SILC gewichtet das Modell nach Haushalten, nicht nach Personen, weil die
Personenzahl je Dezil im Datensatz fehlt. Der \emph{Gini-Koeffizient} ist die Fläche zwischen
Lorenzkurve und Gleichverteilungsgerade im Verhältnis zur ganzen Dreiecksfläche darunter (0 =
...
des mittleren Haushaltsnettos hat (hier ohne Bedarfsgewichtung); weil ein Durchschnitt je
Dezil dafür zu grob ist, wird der Anteil innerhalb der unteren drei Dezile aus dem Status quo
fortgeschrieben.

\begin{merksatz}
Jede Steuer und jede Leistung landet bei einem Haushaltstyp. Gini und Palma werden wie in
der amtlichen Statistik auf Äquivalenzeinkommen gerechnet, die Armutsquote auf das
Haushaltsnetto.
\end{merksatz}
...
\begin{itemize}
\item \emph{Arbeit}: Mehr Arbeitsangebot hebt das BIP ab der Folgeperiode, mit dem
  Arbeitsanteil 0,65 und ohne Anpassungsverzögerung. In der laufenden Periode bewegt nur die
  Nachfrage das BIP.
...
\item \textbf{Restabweichung Haushalt gegen Staat.} Bei Umsatzsteuer und Beiträgen weicht die
  Summe über die Haushalte noch von der Buchung des Staates ab: ohne Nachfrage um bis zu rund
  10\,\%, mit Nachfrage um bis zu rund 15\,\% (die Toleranz in \datei{tests/buchung.test.js}).
  Ein Grund: Die Konsumreaktion auf die Umsatzsteuer kennt nur der Staat.
...
\item \textbf{Die Beitragsbemessungsgrenze wirkt asymmetrisch.} Eine Anhebung um 10.000\,\euro{}
  ...
  Etwa die Hälfte der Verbesserung ist mechanisch: Das höhere Vornetto erhöht den Konsum und
  damit die Umsatzsteuer. Die andere Hälfte kommt über die Nachfrage. Das ist ein
  Buchungsfehler; er ist zur Behebung vorgemerkt.
...
\item \textbf{Die Ausweich- und Wegzugsreaktion des obersten Prozents greift am Tisch nie.}
  Ihre Schwellen (45 und 60\,\%) werden mit dem Grenzsteuersatz auf das
  \emph{Haushaltsbrutto} verglichen, der die zvE-Quote von 0,79 enthält. Auch bei einem
  Spitzensatz von 75\,\% liegt er für D10c nur bei rund 36\,\%. Erst wenn in der klassischen
  Fläche zusätzlich die Grenze der obersten Zone stark gesenkt wird, setzt die Reaktion ein.
  Ob die Schwellen auf den tariflichen Grenzsteuersatz bezogen sein sollten, ist zu prüfen.

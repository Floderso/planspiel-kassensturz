...
Gesamtwirkung. \emph{Innerhalb} einer Periode ist der Graph kreisfrei: Die Nachfrage wirkt
auf BIP und Einnahmen, aber nicht zurück auf Nettoeinkommen, Arbeitsangebot oder
Bemessungsgrundlagen; eine Iteration bis zu einem Fixpunkt gibt es nicht. Kreise entstehen
erst über die Periodengrenze: Schulden erzeugen Zinsen, die den Saldo der nächsten Periode
belasten, und das BIP-Niveau skaliert die Einnahmen der nächsten Periode.
...
\draw[pf] (emi.east) -- node[gl]{$\frac{E}{E^B(\tau)}\sum_j E^B(\tau+j)$ {\tiny entlang des Pfads}} (cob.west);
\draw[pf] (arb.north east) -- node[gl, pos=0.55]{$\tilde B_i=\lambda_iB_i$} (hh.south west);
\draw[rk] (sq.north) |- node[gl, pos=0.3, text=grau]{{\tiny Zinsen $rbY$}\\{\tiny nächste Periode}} ($(sal.east)+(0,0.3)$);
\node[font=\scriptsize, text=grau, align=center] (ynote) at (8.6,-8.25) {$\times y$: alle Einnahmen\\der nächsten Periode};
...
\wert{kanal}{rv}{est}\,\Mrd. Die Umsatzsteuer sinkt, weil das Vornetto und damit der
besteuerte Konsum sinkt und weil die Nachfrage nachlässt; die Einkommensteuer sinkt allein
über die Nachfrage, denn Beiträge mindern im Modell nicht die Bemessungsgrundlage der
Einkommensteuer.
...
das Rentenniveau unmittelbar nur auf die Ausgaben; alle übrigen Kanten laufen über die
Nachfrage.}{fig:soziales}
\ressortgraph{Umwelt}{Ressort Umwelt und Klima. Das Klimageld verschiebt das
\COz-Aufkommen zu 70\,\% zu den Transfers.}{fig:umwelt}
...
\item \textbf{Jede Steuer bewegt mehr als ihren eigenen Kanal.} Eine höhere Umsatzsteuer
  senkt auch Einkommensteuer und Beiträge, und zwar allein über die Nachfrage: Die
  Haushalte haben weniger Kaufkraft, das BIP der Periode sinkt, und mit ihm alle
  BIP-gebundenen Einnahmen. Eine höhere Beitragsrate senkt die Umsatzsteuer zusätzlich
  unmittelbar, weil das Vornetto und damit der besteuerte Konsum sinkt.
...
\input{generiert/gesamtmatrix.tex}
...
Emissionen bewegt allein der \COz-Preis. Das BIP-\emph{Niveau} 2041, also was dauerhaft
bleibt, bewegen nur vier Stellgrößen am Tisch: Grundfreibetrag und Eingangssatz über das
Arbeitsangebot, Körperschaft- und Gewerbesteuer über das private Kapital. Alle anderen wirken
auf das BIP nur über die Nachfrage der jeweiligen Periode, und die verschwindet mit der
Politik.
...
BIP-Niveau \SQbipEnde\,\Mrd, \COz{} kumuliert \SQcoEnde\,Mt.}
\label{tab:pfad}
\end{table}
...
\item Der \textbf{Grundfreibetrag} kostet \wert{p1}{freibetrag}{saldo}\,\Mrd{} im Jahr, erhöht
  ...
  Arbeitsangebot hebt das BIP-Niveau 2041 um \wert{pfad}{freibetrag}{bip}\,\Mrd.
\item Die \textbf{Körperschaftsteuer} bringt \wert{p1}{kst}{saldo}\,\Mrd, senkt die Quote aber
  nur um \wert{pfad}{kst}{schuld}\,\Pp: Das private Kapital schrumpft, das BIP-Niveau 2041 sinkt um
...
\centering\scriptsize
\setlength{\tabcolsep}{3pt}
\input{generiert/kennzahlen.tex}
...
\centering\scriptsize
\setlength{\tabcolsep}{3pt}
\input{generiert/weitere_kennzahlen.tex}
